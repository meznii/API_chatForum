...
const so = require("socket.io")(server)
...
io.on('connection',(socket)=>{
    ...
    if (title == null || content == null) {
      so.in(socket.room).emit('erreur_message', {erreur : 'missing parameters'});
    } else {
    asyncLib.waterfall([
      function(done) {
        userM.findOne(
        ...
        .catch(function(err) {
          so.in(socket.room).emit('erreur_message', {erreur : 'unable to verify user'});
        });
      },
      function(userFound, done) {
        if(userFound) {
          ...
          so.in(socket.room).emit('erreur_message', {erreur : 'user not found'});
        }
      },
    ], function(newMessage) {
      if (newMessage) {
        console.log(newMessage);
        so.in(socket.room).emit('new_message', {message : data.message, username : socket.username});
        return newMessage;
      } else { 
        so.in(socket.room).emit('erreur_message', {erreur : 'cannot post message'});
      }
    });
    ...
    socket.on('message',function(data){
      console.log(data.room)
            io.in(data.room).emit('new_message', {user:data.user, message:data.message, room: data.room});
          })
